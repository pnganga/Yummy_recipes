...
class TestYummyRecipes(TestCase):
	...
	def test_user_recipes(self):
		self.client.get('/user-recipes')
		self.assert_template_used('user-recipes.html')
	...
	def test_new_user_gets_added_to_users(self):
		self.users = simulated_models.Users()
		self.user = {"firstname": "Pius", "lastname": "Ngash", "email": "pnganga05@gmail.com", "password": "apassword"}
		self.users.add_user(self.user)
		self.assertEqual(self.users.users, [self.user])
